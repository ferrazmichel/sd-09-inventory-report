...
def biggest_company(list_prod):
    ...
    return max(companies_big)


class SimpleReport():
    def generate(list_prod):
        order = order_date(list_prod)
        nearest = nearest_date(list_prod)
        company = biggest_company(list_prod)
        return (
